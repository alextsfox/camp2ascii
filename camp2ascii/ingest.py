...
def ingest_tob3_data(input_buff: BufferedReader, header: TOB3Header | TOB2Header, ascii_header_nbytes: int, n_invalid: int | None, pbar: tqdm | None) -> tuple[list[bytes], list[bytes], list[Footer], np.ndarray, list[list[bytes]], list[list[bytes]], list[list[Footer]]]:
    ...
    for _ in range(header.table_nframes_expected + 5):
        ...
        if len(footer_bytes) < FRAME_FOOTER_NBYTES:
            mask[major_frame*header.data_nlines:(major_frame+1)*header.data_nlines] = True
        ...
        if footer.validation not in (header.val_stamp, int(0xFFFF ^ header.val_stamp)):
            ...
            skipped_frames += 1
            mask[major_frame*header.data_nlines:(major_frame+1)*header.data_nlines] = True  # mark the whole frame as missing if the footer is invalid, since we can't trust the minor frame flag
    ...
    return headers_raw[:final_frame], data_raw[:final_frame], footers_raw[:final_frame], mask[:final_frame*header.data_nlines], minor_headers_raw, minor_data_raw, minor_footers_raw
